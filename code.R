        ...
        fluidRow(
          column(
            ...
            box(
              width = NULL,
              status = "primary",
              selectInput("country_mol", "Filter by Country:", 
        ...
        fluidRow(
          column(
            ...
            box(
              width = NULL,
              status = "primary",
              selectInput("country_diag", "Filter by Country:", choices = NULL),
              selectInput("category_diag", "Filter by Category:", choices = NULL)
            )
          ),
          column(
            width = 9,
        ...
        fluidRow(
          column(
            ...
            box(
              width = NULL,
              status = "primary",
              selectInput("country3", "Filter by Country:", 
                          choices = c("Germany", "France", "Italy", "Poland"))
            )
          ),
          column(
            ...
            box(
              width = NULL,
              highchartOutput("otcSalesChartMG", height = "300px"),
        ...
        fluidRow(
          column(
            ...
            box(
              width = NULL,
              status = "primary",
              selectInput("country4", "Filter by Country:", 
                          choices = c("Germany", "France", "Italy", "Poland")),
        ...
        fluidRow(
          column(
            ...
            box(
              width = NULL,
              status = "primary",
              selectInput("country2", "Filter by Country:", choices = NULL),
              selectInput("category2", "Filter by Category:", choices = NULL)
            )
          ),
          column(
            width = 9,
        ...
        fluidRow(
          column(
            ...
            box(
              width = NULL,
              fluidRow(
                column(6, selectInput("country5", "Filter by Country:", choices = NULL)),
                column(6, selectInput("Category5", "Filter by Category:", choices = NULL))
              ),
...
server <- function(input, output, session) {
  ...
  population_factor_path_2 <- 'www/Persona_share_v2.xlsx'
  ...
  otcSalesbuden_point_esitmete_2 <- reactive({
    ...
    result <- x %>% left_join(pop_factor, by = "Category") %>%
      ...
      mutate(Total_mgs_rx = ceiling(Patients_rx_persona1_mg+Patients_rx_persona2_mg+Patients_rx_persona3_mg)) %>%
      mutate(Total_mgs_alt = ceiling(Patients_alt_persona1_mg+Patients_alt_persona2_mg+Patients_alt_persona3_mg)) %>%
      mutate(Total_mgs_not = ceiling(Patients_not_persona1_mg+Patients_not_persona2_mg+Patients_not_persona3_mg)) %>%
      ...
      select(Category,Share_of_population_taking_Rx,Patients_seeking_Rx,Patients_seeking_alternatives,Patients_seeking_nothing,Patients_per_category,
             Patients_rx_persona1_mg,Patients_rx_persona2_mg,Patients_rx_persona3_mg,Total_mgs_rx,
             Patients_alt_persona1_mg,Patients_alt_persona2_mg,Patients_alt_persona3_mg,Total_mgs_alt,
             Patients_not_persona1_mg,Patients_not_persona2_mg,Patients_not_persona3_mg,Total_mgs_not,
             Clinic_visits_from_Rx_pop,Clinic_visits_from_Alt_pop,Clinic_visits_from_not_pop)
    #left_join(y) %>%
    #select(Category, Patients_per_category,Share_of_population_taking_Rx,Patients_seeking_Rx,Patients_seeking_rx_persona1_mg,Patients_seeking_rx_persona2_mg,Patients_seeking_rx_persona3_mg,Total_mgs,PPMG)
    ...
    result <- result %>%
      ...
      mutate(Total_rx_cost_ibuprofen = Cost_rx_ibuprofen_persona_1 +Cost_rx_ibuprofen_persona_2+Cost_rx_ibuprofen_persona_3) %>%
      ...
      mutate(Total_alt_cost_ibuprofen = Cost_alt_ibuprofen_persona_1 +Cost_alt_ibuprofen_persona_2+Cost_alt_ibuprofen_persona_3) %>%
      ...
      mutate(Total_not_cost_ibuprofen = Cost_not_ibuprofen_persona_1 +Cost_not_ibuprofen_persona_2+Cost_not_ibuprofen_persona_3)  %>%
      ...
      mutate(Total_visits = Clinic_visits_from_Rx_pop +Clinic_visits_from_Alt_pop+Clinic_visits_from_not_pop)  %>%
      mutate(Total_visit_cost = Clinic_visit_cost_Rx +Clinic_visit_cost_alt+Clinic_visit_cost_nothing) %>%
      ...
      mutate(Total_health_care_cost = Total_rx_health_care_cost +Total_alt_health_care_cost + Total_not_health_care_cost )%>%
      ...
      mutate(Total_productivity_loss = Prod_loss_cost_Rx +Prod_loss_cost_alt + Prod_loss_cost_nothing  )%>%
      ...
      mutate(Total_cost = Total_Rx +Total_alt + Total_nothing  )
  ...
  output$Total_cost <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Total_Rx","Total_alt","Total_nothing","Total_cost"), ]
  ...
  output$waterfall_prodcutivity_related_cost <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Prod_loss_cost_Rx","Prod_loss_cost_alt","Prod_loss_cost_nothing","Total_productivity_loss"), ]
  ...
  output$waterfall_total_health_care_cost <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Total_health_care_cost","Total_rx_health_care_cost", "Total_alt_health_care_cost","Total_not_health_care_cost"), ]
  ...
  output$waterfall_patient_visits <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Clinic_visits_from_Rx_pop","Clinic_visits_from_Alt_pop", "Clinic_visits_from_not_pop","Total_visits"), ]
  ...
  output$waterfall_patient_visits_costs <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Clinic_visit_cost_Rx","Clinic_visit_cost_alt", "Clinic_visit_cost_nothing","Total_visit_cost"), ]
  ...
  output$waterfall_patient_population <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Patients_per_category","Patients_seeking_Rx", "Patients_seeking_alternatives", "Patients_seeking_nothing"), ]
  ...
  output$waterfall_ibuprofen_cost_rx <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Cost_rx_ibuprofen_persona_1", "Cost_rx_ibuprofen_persona_2", "Cost_rx_ibuprofen_persona_3","Total_rx_cost_ibuprofen"), ]
  ...
  output$waterfall_ibuprofen_cost_alt <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Cost_alt_ibuprofen_persona_1", "Cost_alt_ibuprofen_persona_2", "Cost_alt_ibuprofen_persona_3","Total_alt_cost_ibuprofen"), ]
  ...
  output$waterfall_ibuprofen_cost_not <- renderHighchart({
    ...
    waterfall_data <- otcSalesbuden_point_esitmete_2()
    waterfall_data <- waterfall_data[waterfall_data$name %in% c("Cost_not_ibuprofen_persona_1", "Cost_not_ibuprofen_persona_2", "Cost_not_ibuprofen_persona_3","Total_not_cost_ibuprofen"), ]
  ...
  output$otc_mol_chart <- renderHighchart({
    ...
    highchart() %>%
      hc_chart(type = "bar") %>%
      hc_title(text = "Total molecules containing ibuprofen in target markets") %>%
      hc_xAxis(categories = data$`Molecule Combination`, title = list(text = "Molecule") , labels = list(style = list(color = 'black'))) %>%
      hc_yAxis(title = list(text = "Total MG")) %>%
      hc_plotOptions(
        series = list(
          colorByPoint = TRUE,
